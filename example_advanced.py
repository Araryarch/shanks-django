...
def get_profile(req: Request):
    ...
    return {"username": user.username, "email": user.email, "visits": visits + 1}
...
def dashboard(req: Request):
    return Response().render(
        req.django, "dashboard.html", {"user": req.user, "data": {"title": "Dashboard"}}
    )
...
def advanced(req: Request):
    ...
    return {"ip": ip, "user_agent": user_agent, "total_users": users.count()}
