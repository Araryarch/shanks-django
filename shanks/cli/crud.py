...
def create_crud_endpoint():
    ...
    endpoint_plural = endpoint_name + "s" if not endpoint_name.endswith("s") else endpoint_name
    ...
    repository_file.write_text(
        get_repository_template(model_name, endpoint_name),
        encoding="utf-8"
    )
    ...
    routes_file = routes_dir / f"{endpoint_name}_route.py"
    routes_file.write_text(
        get_route_template(model_name, endpoint_name, endpoint_plural),
        encoding="utf-8"
    )
