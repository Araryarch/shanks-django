...
def get_{endpoint_name}_by_id(item_id):
    ...
    if operations.get("update"):
        functions.append(
            f'''def update_{endpoint_name}(item_id, title=None, description=None):
    """Update existing item"""
    item = {endpoint_name}_repository.find_by_id(item_id)
    if not item:
        return None
...
def get_controller_template(
    model_name, endpoint_name, endpoint_plural, operations=None
):
...
def get_route_template(
    model_name, endpoint_name, endpoint_plural, operations=None, require_auth=False
):
    """Route template"""
    if operations is None:
        operations = {"create": True, "read": True, "update": True, "delete": True}
    ...
    # Build imports
    imports = [
        f"from shanks import App",
        f"from internal.controller import {endpoint_name}_controller",
    ]
    if require_auth:
        imports.append(
